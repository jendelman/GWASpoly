#' Prepare GWASpoly dosage input file from VCF file
#' 
#' Prepare GWASpoly dosage input file from VCF file
#' 
#' The "GT" option for \code{geno.code} is the posterior maximum genotype (e.g., 0/0/1/1), while "DS" represents the posterior mean dosage of the alternate allele. (At present, only bi-allelic variants are supported.) The VCF file must conform to version 4.1 of the VCF specification or later. 
#'
#' @param VCF.file VCF filename to read in
#' @param dosage.file name of CSV file to output with allele dosage
#' @param geno.code genotype code in the FORMAT field: either "GT" or "DS" (see Details)
#' @param ploidy ploidy
  ...
  while(length(temp) > 0) {
    ...
    if (m1%%1e4==0) {cat(sub("X",m1,"Number of variants processed...X\n"))}
  	...
  	if (max(regexpr(",",c(REF,ALT),fixed=T)) < 0) {
		  ...
		  if (geno.code=="DS")
		    geno <- sapply(data,function(w){as.numeric(w[geno.pos])})
		  if (geno.code=="GT") 
		    geno <- sapply(data,function(w){
		      if(length(w) < geno.pos || w[geno.pos]==".") {
		        return(as.numeric(NA))  
		      } else {
		        y <- strsplit(w[geno.pos],split="/",fixed=T)[[1]]
